...
monthly_bill = sample(c(250, 300, 350, 150, 100, 155), 10, replace = T)

salary = sample(c(24000, 56000, 125000, 92000, 75000, 50200), 10, replace = T)

price_elec = sample(c(0.15, 0.05, 0.45, 0.25, 0.30), 10, replace = T)

housingtype = sample(c("Single-Family Detached", "Apartment 2-4", "Apartment 5 or more", "Mobile", "Single-Family Attached"), 10, replace = T)

climate = sample(c("Very Cold","Mixed-humid","Mixed-dry","Hot-humid","Marine"), 10, replace = T)

ownership = sample(c("Rented","Owned"), 10, replace = T)

heating_fuel = sample(c("gas","electric"), 10, replace = T)

test_tibble = tibble(users, regions, salary, monthly_bill, price_elec, housingtype, climate, ownership, heating_fuel)
...
server <- function(input, output) {
  ...
  output$finance_plot <- renderPlot({
    ggplot(test_tibble, aes(x = ownership, y = currEn)) +
      geom_boxplot()
  })
}
